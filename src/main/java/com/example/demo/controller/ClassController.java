...
@RestController("/class") // 將此類別設定為api接口類別, 並返回ＪＳＯＮ格式的 元件標籤（@Component), 有貼標籤的類別無法另外用 new
public class ClassController {

    @Autowired  // 將類別或介面注入 的標籤
    private ClassService classService;

    // 掛在api 方法上（可設定路徑, 方法, 或者把路徑拿來當變數）
    @RequestMapping(value = "/getUserClassData",method = RequestMethod.GET)
